...
namespace DigitalOceanUploader.Shared
{
    ...
    public class DigitalOceanUploadManager : IDisposable
    {
        ...
        public DigitalOceanUploadManager(string accessKey, string secretKey, string spaceName, string serviceUrl = "https://sfo2.digitaloceanspaces.com/")
        ...
        /// <param name="spaceName">Space name.</param>
        /// <param name="serviceUrl">Service URL.</param>
        public DigitalOceanUploadManager(KeyManager keyManager, string spaceName, string serviceUrl = "https://sfo2.digitaloceanspaces.com/")
            : this(spaceName, serviceUrl)
        {
            if (keyManager == null)
        ...
        private DigitalOceanUploadManager(string spaceName, string serviceUrl = "https://sfo2.digitaloceanspaces.com/")
        {
        ...
        public async Task<string> UploadFile(string filePath, string uploadName, int maxPartRetry = 3, long maxPartSize = 6000000L)
        {
            ...
            using (var client = CreateNewClient())
            {
                var meta = new Amazon.S3.Model.MetadataCollection();
                var mpUploadReq = new Amazon.S3.Model.InitiateMultipartUploadRequest()
                {
                    BucketName = _spaceName,
                    ContentType = contentType,
                    Key = uploadName
                };
                mpUploadReq.Headers["x-amz-acl"] = "public-read";
                //mpUploadReq.Metadata.Add("x-amz-acl", "public-read");
                multiPartStart = await client.InitiateMultipartUploadAsync(mpUploadReq);
